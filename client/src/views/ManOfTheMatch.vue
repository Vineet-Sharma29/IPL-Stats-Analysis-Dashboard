...
<script>
  ...
  export default {
    ...
    methods: {
      requestData(){
        this.loaded = false
        axios.get('http://localhost:3000/man-of-the-match')
        .then(response => {
         
          this.datacollection = response.data.slice(0,100).map( a => a.y_label )
          this.options = response.data.slice(0,100).map( a => a.x_label )
        })
      ...
      fireQuery(prop){
        ...
        if( prop == 'All Time' ){
            ...
            .then(response => {
              ...
              this.options = response.data.slice(0,50).map( a => a.x_label )
            })
            .finally( ()=> this.loaded = true )
            ...
            axios.post('http://localhost:3000/man-of-the-match/year', {year:prop})
            .then(response => {
              this.datacollection = response.data.slice(0,10).map( a => a.y_label )
              this.options = response.data.slice(0,10).map( a => a.x_label )
            })
            .finally( ()=> this.loaded = true )
            this.desc = prop
        }
